...
# Interactive parameter input for call option analysis
print("\nEuropean Call Option Implied Volatility & Greeks Calculator (Black-Scholes)")
...
if __name__ == "__main__":
    ...
    # Market price of the call option (do not ask, use from Yahoo)
    try:
        call_row = opt_chain.calls[opt_chain.calls['strike'] == K]
        if not call_row.empty:
            C_market = float(call_row['lastPrice'].values[0])
            print(f"Market price for selected call: {C_market}")
        else:
            C_market = 1.0
    except Exception:
        C_market = 1.0
    ...
    theoretical_price = black_scholes_call_price(S, K, T, r, sigma)
    ...
    print(f"{'Market Call Price:':<25}${C_market:.2f}")
    print(f"{'Theoretical Call Price:':<25}${theoretical_price:.2f}")
...
def implied_volatility_call(S, K, T, r, market_price):
    return implied_volatility_newton(market_price, S, K, T, r)