...
def finite_difference_greeks_call(S0, K, T, r, sigma, Smax=3, M=100, N=1000, bump=1e-5, debug=False):
    """
    Calculate Greeks for a European call option using finite differences (bumping) on the finite difference price.
    Returns a dict with Delta, Gamma, Vega, Theta, Rho.
    """
    # Delta
    dS = S0 * bump
    price_up = finite_difference_european_call(S0 + dS, K, T, r, sigma, Smax, M, N)
    price_down = finite_difference_european_call(S0 - dS, K, T, r, sigma, Smax, M, N)
    price = finite_difference_european_call(S0, K, T, r, sigma, Smax, M, N)
    delta = (price_up - price_down) / (2 * dS)
    gamma = (price_up - 2 * price + price_down) / (dS ** 2)
    # Vega
    dsigma = bump
    price_vega = finite_difference_european_call(S0, K, T, r, sigma + dsigma, Smax, M, N)
    vega = (price_vega - price) / dsigma
    # Theta (should be negative for calls)
    dT = min(bump, T/10) if T > 0.01 else 1e-6
    if T - dT > 0:
        price_theta = finite_difference_european_call(S0, K, T - dT, r, sigma, Smax, M, N)
        theta = (price_theta - price) / dT  # Correct sign: should be negative
    else:
        theta = float('nan')
    # Rho
    dr = bump
    price_rho = finite_difference_european_call(S0, K, T, r + dr, sigma, Smax, M, N)
...
if __name__ == "__main__":
    ...
    greeks = finite_difference_greeks_call(S0, strike, T, r, sigma, debug=True)
